...
import { basename } from '@renderer/utils/basename';
...
const tabSchema = z.preprocess((s) => parseInt(s as string), z.number().min(0)).catch(-1);
            ...
            <Show when={filePath.includes(search().trim())}>
              ...
              >
                ...
                <span class="absolute -bottom-6 max-w-full truncate text-teal-300 opacity-0 transition-opacity group-hover:opacity-100">
                  {basename(filePath, api.isWindows)}
                </span>
                <Link
